...
var engine,world,ground,ball,rope,hour;
var s=0;
var bg="orange"
function preload(){
  getTime();
...
function setup() {
  ...
  //CREATING GROUND
    ...
    g2 = new Gru(29*width/48,((3*height/5)-(4*height/50)-(height/60)),4*width/25,1)
    g3 = new Ground(4.3*width/5,2*height/5,width/5,height/30)
    g4 = new Gru(4.3*width/5,((2*height/5)-(4*height/50)-(height/60)),4*width/25,1)
...
function draw() {
 // if(bg)
  ...
  textSize(30)
   strokeWeight(2)
      stroke("white");
  text("times ball used :- "+ s,100,100)
